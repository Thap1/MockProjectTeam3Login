package com.spring.cmc.services.impl;

import java.util.ArrayList;
import java.util.List;
import java.util.Optional;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.web.bind.annotation.RequestBody;

import com.spring.cmc.entity.Menu;
import com.spring.cmc.repository.MenuRepository;
import com.spring.cmc.service.MenuService;



@Service
public class MenuServiceImpl implements MenuService{
	
@Autowired
	
	private MenuRepository menuRepository;

@Override
public List<Menu> getAllMenu() {
	
	List<Menu> list = new ArrayList<>();
	menuRepository.findAll().forEach(list::add);
	return list;
}

@Override
public Optional<Menu> findById(int id) {
	Optional<Menu> menu = menuRepository.findById(id);
	return menu;
}

@Override
public Menu saveMenu(Menu object) {
	
	return menuRepository.save(object);
}

@Override
public void deleteMenuById(int id) {
	
	
	menuRepository.deleteById(id);
	
}

@Override
public void deleteAllMenu() {
	
	menuRepository.deleteAll();
}

@Override
public List<Menu> findByMenuNameContaining(String name) {
	
	List<Menu> listmenu= menuRepository.findByMenuNameContaining(name);
	return listmenu;
	
}

@Override
public Menu findByMenuId(int menuId) {
	
	Menu menu = menuRepository.findByMenuId(menuId);
	return menu;
	
}

@Override
public Menu createMenu(Menu menu) {
	// TODO Auto-generated method stub
	return null;
}

//@Override
//public Menu createMenu(Menu menu) {
//	
//	Menu newMenu = menuRepository.save(new Menu(0,menu.getMenuName(), menu.getMenuDescription(), menu.getMenuFunction()));
//	return newMenu;
//
//}





}
